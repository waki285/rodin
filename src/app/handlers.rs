...
pub async fn security_middleware(mut req: Request<Body>, next: Next) -> Response {
    ...
    let csp = format!(
        "default-src 'self'; script-src 'self' 'nonce-{}' 'unsafe-inline' static.cloudflareinsights.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self' cloudflareinsights.com; object-src 'none'; frame-ancestors 'self'; base-uri 'self'; require-trusted-types-for 'script'",
        nonce
    );
